...
$(document).ready(function () {
    var table = $('#InonPaymentTable').DataTable({
        ...
        },
        {
            ...
            "className": "text-center",
            'render': function (data, type, full, meta) {
                return `<a href="#" onclick="openCooEditWindow('/outShow/${data}/'); return false;"><i class="fa-regular fa-eye" style="color: #ff0000;"></i></a>`
            }
        },
        {
            "width": "50px",
...
function TransmitDataClick() {
    let ArrayVal = [];
    var ChechValue = document.getElementsByName("ListAllCheckSubmitId");
    let TrasnMitValue = $("#InnonPaymentTransmitData").val()
    ChechValue.forEach(
        function (Val) {
            if (Val.checked) {
